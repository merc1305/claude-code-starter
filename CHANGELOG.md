...
### Добавлено
- **Slash-команды пайплайна планирования** (`.claude/commands/`, едут с репозиторием): `/plan` - диспетчер, сканирует `plans/`, читает статус из шапки плана и плашками (AskUserQuestion) ведёт по этапам `черновик → критика фаз → best-check → premortem → реализация`; точечные - `/new-plan`, `/plan-phases`, `/plan-critique`, `/plan-best`, `/plan-premortem`.
- **Каноны этапов** в `prompts/methodology/`: новые `new-plan.md`, `plan-best.md`, `plan-premortem.md`; `plan-critique.md` расширен 4 контрольными вопросами по каждой фазе и веб-разведкой рынка.
- **Статусы этапов в шаблоне плана** (`plans/TEMPLATE.md`): статус живёт в шапке файла, вердикты - в новой секции «Журнал этапов»; отдельного реестра нет - файл плана единственный источник правды.

### Безопасность
- Правило «секреты без fallback» в разделе «Безопасность» `CLAUDE.md`: запрет дефолтов для `JWT_SECRET`, ключей шифрования и паролей БД (`?? 'dev-secret'`, добивка ключа нулями и т.п.). Без обязательной env-переменной приложение должно падать при старте, а не молча запускаться на проде с секретом из публичного репо (иначе - тривиальный обход авторизации). Наблюдение с реального форка стартера: Claude по умолчанию генерирует такие «удобные» fallback'и, если это не запрещено явно.
